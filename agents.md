...
- **Writeback Agents**: `src/write/writeback.ts`, `src/write/summarize_note.ts`, `src/write/tag_note.ts`, and `src/write/related_content.ts` encapsulate safe note updates. All AI output is wrapped with `<!-- AI:BEGIN --> ... <!-- AI:END -->` markers, which the indexer automatically strips before chunking. Summaries output recap + recommendations, the tagger emits universal tags (#project-management, #health, etc.), and the related-content agent appends wiki links to similar files at the bottom of notes. Batch wrappers (`tag_all`, `summarize_all`, `related_all`, `enrich_all`) are available in `src/write/` alongside `enrich_note.ts`, which chains the three writebacks for a single file.
  - Every script uses the shared `logger` for structured output—no raw `console.*` usage—so logs stay uniform.
  - Type safety is enforced throughout; we avoid `any` and rely on declarative helpers (e.g., dedupe functions, frontmatter utilities) rather than ad-hoc skipping logic.
